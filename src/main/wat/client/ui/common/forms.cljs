(ns wat.client.ui.common.forms
  (:require [com.fulcrologic.fulcro.algorithms.form-state :as fs]
            ...
            [wat.client.application :refer [SPA]]
              ...
              (fn [{::uism/keys [state-map] :as env}]
                ...
                (let [FormClass (uism/actor-class env :actor/form)
                  ...
                  (if-not save-mutation
                    (log/info (str "Class " FormClass " must have a 'wat.client.common.forms/save-mutation"))
                    (-> env
                        (uism/assoc-aliased :busy? true)
             ...
             {::uism/handler
              (fn [env]
                (let [proceed (js/confirm "Really delete?")
                  ...
                  (cond
                    (not delete-mutation)
                    (log/error (str "Class " FormClass " must have a 'wat.client.common.forms/delete-mutation"))
       ...
       (fn [{::uism/keys [state-map] :as env}]
         ...
         (let [FormClass (uism/actor-class env :actor/form)
           ...
           (if-not create-mutation
             (log/error (str "Class " FormClass " must have a 'wat.client.common.forms/create-mutation"))
             (-> env
                 (uism/assoc-aliased :busy? true)
