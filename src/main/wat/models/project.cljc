(ns wat.models.project
  (:require [clojure.set :refer [rename-keys]]
            [com.wsscode.pathom.connect :as pc]
            [com.fulcrologic.fulcro.algorithms.form-state :as fs]
            [com.fulcrologic.fulcro.mutations :as m]
            #?(:clj [wat.xtdb.project :as prj])
            #?(:clj [wat.xtdb.project-config :as prjc])
            #?(:clj [wat.xtdb.easy :as gxe])
            #?(:clj [wat.xtdb.util :as xutil])
            #?(:clj [wat.models.common :as mc])
            #?(:clj [wat.models.auth :as ma])
            #?(:clj [wat.xtdb.user :as usr])))
   ...
   (pc/defresolver get-project [{:keys [node]} {:project/keys [id]}]
     {::pc/input     #{:project/id}
      ::pc/output    [:project/id :project/name :project/readers :project/writers
                      :project/documents :project/config]
